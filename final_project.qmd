...
```{python}
# Merge datasets
state_preschool_stats = spending.merge(enrollment, on=['Year', 'State Name', 'Program Name'], how='inner')
state_preschool_stats = state_preschool_stats.merge(quality_standards, on=['Year', 'State Name', 'Program Name'], how='inner')
```

```{python}
# See if Illinois has enough data to be interesting

illinois_preschool = state_preschool_stats[state_preschool_stats['State Name']=='Illinois']
...
```{python}
spending_wide = spending.pivot_table(index=['State Name', 'Year'], columns='Variable Name', values='Spending (2023 Dollars)', aggfunc='sum').reset_index()
...
alt.Chart(spending_wide).mark_line().encode(
   alt.X('Year'),
   alt.Y('Total State Pre-K Spending (2023 Dollars)', format='$'),
...
enrollment_wide = enrollment.pivot_table(index=['State Name', 'Year'], columns='Variable Name', values='Enrollment', aggfunc='sum').reset_index()
...
# make year datetime
enrollment_wide['Year'] = pd.to_datetime(enrollment_wide['Year'], format='%Y')

# save national information elsewhere
national_enrollment = enrollment_wide[enrollment_wide['State Name']=='National']

# remove national information from main dataframe to make chart easier to read
enrollment_wide = enrollment_wide[enrollment_wide['State Name']!='National']
...
```{python}
il_preschool_stats = illinois_spending_wide.merge(illinois_enrollment, on=['Year', 'State Name'], how='inner')
```
...
```
```{python}
quality_standards['Program Name'] = quality_standards['Program Name'].fillna('Missing')
quality_summary = quality_standards.groupby(['State Name', 'Year']).agg(
    avg_benchmarks_met=('Standards Met', 'mean'),
    sum_benchmarks=('Standards Met', 'sum'),
    total_benchmarks=('Benchmark Met?', 'count')
).reset_index()
quality_summary['Year'] = pd.to_datetime(quality_summary['Year'], format='%Y')
quality_summary['percent_benchmarks'] = quality_summary['sum_benchmarks']/quality_summary['total_benchmarks']
```

```{python}
il_quality = quality_summary[quality_summary['State Name']=='Illinois']
...
preschool_stats = preschool_stats.merge(quality_summary, on=['Year', 'State Name'], how='inner')
folder_path = 'data'
