...
preschool_geometry = preschool_geometry.rename(columns={
    'Year': 'Date'})
...
```{python}
preschool_geometry.columns
states
```
```{python}
preschool_geometry['Date'] = pd.to_datetime(preschool_geometry['Date'])
preschool_geometry['Year'] = preschool_geometry['Date'].dt.year
